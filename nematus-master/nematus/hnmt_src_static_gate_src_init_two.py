...
from data_iterator_src_hist import TextIterator
...
# batch preparation
def prepare_data(seqs_x, seqs_xp, seqs_y, hist_len, maxlen=None, n_words_src=30000,
                 n_words=30000):
    # x: a list of sentences
    lengths_x = [len(s) for s in seqs_x]
    lengths_xp = [[len(ss) for ss in s] for s in seqs_xp]
    ...
    if maxlen is not None:
        ...
        new_lengths_xp = []
        new_lengths_y = []
        for l_x, l_xp, s_x, s_xp, l_y, s_y in zip(lengths_x, lengths_xp, seqs_x, seqs_xp, lengths_y, seqs_y):
            if l_x < maxlen and l_y < maxlen:
                tmp_seqs_xp, tmp_lengths_xp=[],[]
                for ss_xp,ll_xp in zip(s_xp,l_xp):
                    if ll_xp < maxlen:
                        tmp_seqs_xp.append(ss_xp)
                        tmp_lengths_xp.append(ll_xp)
                # if len(tmp_seqs_xp)>0:
                new_seqs_xp.append(tmp_seqs_xp)
                new_lengths_xp.append(tmp_lengths_xp)
    ...
    maxlen_xp = numpy.max([j for i in lengths_xp for j in i]) + 1
    maxlen_y = numpy.max(lengths_y) + 1

    # print 'maxlen_x:',maxlen_x
    # print 'maxlen_xp:',maxlen_xp

    x = numpy.zeros((n_factors, maxlen_x, n_samples)).astype('int64')
    xp = numpy.zeros((n_factors, maxlen_xp, hist_len,n_samples)).astype('int64')
    y = numpy.zeros((maxlen_y, n_samples)).astype('int64')
    x_mask = numpy.zeros((maxlen_x, n_samples)).astype('float32')
    xp_mask = numpy.zeros((maxlen_xp, hist_len,n_samples)).astype('float32')
    hist_mask = numpy.zeros((hist_len,n_samples)).astype('float32')

    y_mask = numpy.zeros((maxlen_y, n_samples)).astype('float32')
    for idx, [s_x, s_xp, s_y] in enumerate(zip(seqs_x, seqs_xp, seqs_y)):
        x[:, :lengths_x[idx], idx] = zip(*s_x)
        x_mask[:lengths_x[idx]+1, idx] = 1.
        y[:lengths_y[idx], idx] = s_y
        y_mask[:lengths_y[idx]+1, idx] = 1.
        # hist_mask[hist_len-len(s_xp):,idx] = 1.
        hist_mask[:len(s_xp),idx] = 1.

        for ss_idx, ss_xp in enumerate(s_xp):
            xp[:, :lengths_xp[idx][ss_idx], ss_idx, idx] = zip(*ss_xp)
            xp_mask[:lengths_xp[idx][ss_idx]+1, ss_idx, idx] = 1.

    return x, x_mask, xp, xp_mask, hist_mask, y, y_mask
...
def gru_layer(tparams, state_below, options, prefix='gru', mask=None,
              emb_dropout=None,
              rec_dropout=None, init_states=None,
              **kwargs):
...
def param_init_gru_cond(options, params, prefix='gru_cond',
    ...
    # hist:
    W_hist = norm_weight(dim, dim*2)
    params[_p(prefix, 'W_hist')] = W_hist

    Wx_hist = norm_weight(dim, dim)
    params[_p(prefix, 'Wx_hist')] = Wx_hist

    W_ctx_h = norm_weight(dim,dim*3)
    params[_p(prefix, 'W_ctx_h')] = W_ctx_h

    W_ctx_c = norm_weight(dim*2,dim*3)
    params[_p(prefix, 'W_ctx_c')] = W_ctx_c

    b_ctx = numpy.zeros((dim*3,)).astype('float32')
    params[_p(prefix, 'b_ctx')] = b_ctx

    W_ctx_hist = norm_weight(dim,dim*3)
    params[_p(prefix, 'W_ctx_hist')] = W_ctx_hist

    return params


def gru_cond_layer(tparams, state_below, options, hist, prefix='gru',
    ...
    # projected context
    print 'Current dim is:',context.ndim
    ...
    #gru + attention decoder main
    def _step_slice(m_, x_, xx_, h_, ctx_, alpha_, pctx_, cc_, rec_dropout, ctx_dropout,
                    U, Wc, W_comb_att, U_att, c_tt, Ux, Wcx,
                    U_nl, Ux_nl, b_nl, bx_nl, hist, W_hist, Wx_hist, W_ctx_h, W_ctx_c, b_ctx, W_ctx_hist):
    #h_: target hinddern
    #m_: mask
    #x_,xx_: emb
    #h1: hidden for attention
    #pctx_: previous attention
    #ctx_: current attention
     ...
     # h1 = x_m[:, None] * h1 + (1. - x_m)[:, None] * h_tm1
        ...
        gate = tensor.nnet.sigmoid(tensor.dot(h1, W_ctx_h) + tensor.dot(ctx_, W_ctx_c) + tensor.dot(hist, W_ctx_hist) + b_ctx)

        gate1 = _slice(gate, 0, 2*dim)

        preact2 = tensor.dot(h1*rec_dropout[3], U_nl)
        preact2 += tensor.dot(ctx_*ctx_dropout[2], Wc)
        preact2 = (1.-gate1) * preact2
        preact2 += gate1 * tensor.dot(hist, W_hist) + b_nl
        ...
        gate2 = _slice(gate, 2, 1*dim)

        preactx2 = tensor.dot(h1*rec_dropout[4], Ux_nl)
        preactx2 *= r2
        preactx2 += tensor.dot(ctx_*ctx_dropout[3], Wcx)
        preactx2 = (1.-gate2) * preactx2
        preactx2 += gate2 * tensor.dot(hist, Wx_hist) + bx_nl
    ...
    shared_vars = [tparams[_p(prefix, 'U')],
                   ...
                   hist,
                   tparams[_p(prefix, 'W_hist')],
                   tparams[_p(prefix, 'Wx_hist')],
                   tparams[_p(prefix, 'W_ctx_h')],
                   tparams[_p(prefix, 'W_ctx_c')],
                   tparams[_p(prefix, 'b_ctx')],
                   tparams[_p(prefix, 'W_ctx_hist')]
                   ]
...
def init_params(options):
    ...
    params['Wemb_uhist'] = norm_weight(options['n_words_src'], options['dim_word'])
    ...
    params = get_layer(options['encoder'])[0](options, params,
                                              prefix='encoder_uttr_xp',
                                              nin=options['dim_word'],
                                              dim=options['dim'])

    params = get_layer(options['encoder'])[0](options, params,
                                              prefix='encoder_hist_xp',
                                              nin=options['dim'],
                                              dim=options['dim'])
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_state',
                                nin=ctxdim+options['dim'], nout=options['dim']) #+options['dim']
    # decoder
    ...
    params = get_layer('ff')[0](options, params, prefix='ff_logit_ctx',
                                ...
                                ortho=False)
    # params = get_layer('ff')[0](options, params, prefix='ff_logit_hist',
    #                             nin=options['dim'], nout=options['dim_word'],
    #                             ortho=False)
...
def build_model(tparams, options):
    ...
    # n_timesteps = max length; n_samples = batch size
    ...
    # history
    # xp (?, ulen, batch) => (?, ulen, hlen, batch)
    # reshape (?, ulen*hlen, batch)
    xp = tensor.tensor4('xp', dtype='int64') # the input is 4 dims
    xp_mask = tensor.tensor3('xp_mask', dtype='float32')
    # n_timesteps_xp = max length of utternace; n_samples_xp = batch size
    n_timesteps_xp = xp.shape[1]
    hist_len_xp = xp.shape[2]
    n_samples_xp = xp.shape[3] # batch
    # xp = xp.reshape([xp.shape[0], n_timesteps_xp, n_samples_xp*hist_len_xp])


    hist_mask = tensor.matrix('hist_mask', dtype='float32')
    ...
    if options['use_dropout']:
        ...
        rec_dropout_uttr_xp = shared_dropout_layer((2, n_samples_xp*hist_len_xp, options['dim']), use_noise, trng, retain_probability_hidden)
        rec_dropout_hist_xp = shared_dropout_layer((2, n_samples_xp, options['dim']), use_noise, trng, retain_probability_hidden)
        ...
        emb_dropout_uttr_xp = shared_dropout_layer((2, n_samples_xp*hist_len_xp, options['dim_word']), use_noise, trng, retain_probability_emb)
        emb_dropout_hist_xp = shared_dropout_layer((2, n_samples_xp, options['dim']), use_noise, trng, retain_probability_emb)
        ...
        source_dropout_xp = shared_dropout_layer((n_timesteps_xp, n_samples_xp*hist_len_xp, 1), use_noise, trng, retain_probability_source)
        source_dropout_xp = tensor.tile(source_dropout_xp, (1,1,options['dim_word']))
    else:
        rec_dropout = theano.shared(numpy.array([1.]*2, dtype='float32'))
        rec_dropout_r = theano.shared(numpy.array([1.]*2, dtype='float32'))
        rec_dropout_d = theano.shared(numpy.array([1.]*5, dtype='float32'))
        rec_dropout_uttr_xp = theano.shared(numpy.array([1.]*2, dtype='float32'))
        rec_dropout_hist_xp = theano.shared(numpy.array([1.]*2, dtype='float32'))
        ...
        emb_dropout_uttr_xp = theano.shared(numpy.array([1.]*2, dtype='float32'))
        emb_dropout_hist_xp = theano.shared(numpy.array([1.]*2, dtype='float32'))

        ctx_dropout_d = theano.shared(numpy.array([1.]*4, dtype='float32'))

    # word embedding for history
    emb_xp = tparams['Wemb_uhist'][xp.flatten()]
    emb_xp = emb_xp.reshape([n_timesteps_xp, n_samples_xp*hist_len_xp, options['dim_word']])
    if options['use_dropout']:
        emb_xp *= source_dropout_xp

    # encoder='gru', gru_layer() init a set of parameters for foward rnn
    proj_uttr_xp = get_layer(options['encoder'])[1](tparams, emb_xp, options,
                                            prefix='encoder_uttr_xp',
                                            mask = xp_mask.reshape([n_timesteps_xp, n_samples_xp*hist_len_xp]),
                                            emb_dropout=emb_dropout_uttr_xp,
                                            rec_dropout=rec_dropout_uttr_xp)

    #(batch*hlen, dim)
    proj_uttr_xp = proj_uttr_xp[0]
    proj_uttr_xp = proj_uttr_xp.reshape([n_timesteps_xp, hist_len_xp, n_samples_xp, options['dim']])
    proj_hist_xp = proj_uttr_xp[-1]
    print 'proj_uttr_xp_dim:',proj_hist_xp.ndim
    # print 'proj_uttr_xp_shape:',proj_hist_xp.shape
    #(hlen, batch, dim)
    # proj_hist_xp = proj_uttr_xp.reshape([hist_len_xp, n_samples_xp, options['dim']])

    #xp_mask (?, ulen, batch*hlen)
    # xp_mask = xp_mask[-1]
    # xp_mask = xp_mask.reshape([hist_len_xp, n_samples_xp, options['dim']])

    proj_hist_xp = get_layer(options['encoder'])[1](tparams, proj_hist_xp, options,
                                            prefix='encoder_hist_xp',
                                            mask=hist_mask,
                                            emb_dropout=emb_dropout_hist_xp,#todo
                                            rec_dropout=rec_dropout_hist_xp)#todo
    #(batch, dim) #todo: dim change
    proj_hist_xp = proj_hist_xp[0][-1]
    print 'proj_hist_xp_dim:',proj_hist_xp.ndim
    # print 'proj_hist_xp_shape:',proj_hist_xp.shape
    ...
    # encoder='gru', param_init_gru() init a set of parameters for foward rnn
    proj = get_layer(options['encoder'])[1](tparams, emb, options,
                                            prefix='encoder',
                                            mask=x_mask,
                                            emb_dropout=emb_dropout, 
                                            rec_dropout=rec_dropout,
                                            init_states=proj_hist_xp)
    ...
    # encoder='gru', param_init_gru() init a set of parameters for backward rnn
    ...
    #(maxlen, batch, hidden_dim)
    # context will be the concatenation of forward and backward rnns
    ctx = concatenate([proj[0], projr[0][::-1]], axis=proj[0].ndim-1)##(maxlen, batch, hidden_dim*2)

    # mean of the context (across time) will be used to initialize decoder rnn
    #(batch, hidden_dim*2)
    ctx_mean = (ctx * x_mask[:, :, None]).sum(0) / x_mask.sum(0)[:, None]
    # print 'ctx_mean_dim',ctx_mean.

    # or you can use the last state of forward + backward encoder rnns
    # ctx_mean = concatenate([proj[0][-1], projr[0][-1]], axis=proj[0].ndim-2)

    # concatenate(ctx_mean + context)
    #(11, 162) (11, 81) =>(11, 243)
    ctx_mean = concatenate([ctx_mean,proj_hist_xp],axis=proj_hist_xp.ndim-1)

    if options['use_dropout']:
        ctx_mean *= shared_dropout_layer((n_samples, 3*options['dim']), use_noise, trng, retain_probability_hidden)

    # initial decoder state, fflayer()
    ...
    # decoder - pass through the decoder conditional gru with attention, gru_cond_layer()
    # return h2, ctx_, alpha.T
    proj = get_layer(options['decoder'])[1](tparams, emb, options, proj_hist_xp,
                                            prefix='decoder',
                                            mask=y_mask, context=ctx,
                                            context_mask=x_mask,
                                            one_step=False,
    ...
    # logit_hist = get_layer('ff')[1](tparams, proj_hist_xp, options,
    #                                prefix='ff_logit_hist', activ='linear')

    # logit = tensor.tanh(logit_lstm+logit_prev+logit_ctx+logit_hist)
    ...
    #print opt_ret
    return trng, use_noise, x, x_mask, y, y_mask, xp, xp_mask, hist_mask, opt_ret, cost, proj_hist_xp
...
def build_sampler(tparams, options, use_noise, trng, return_alignment=False):
    ...
    xp = tensor.tensor4('xp', dtype='int64')
    n_timesteps_xp = xp.shape[1]
    hist_len_xp = xp.shape[2]
    n_samples_xp = xp.shape[3]
    xp_mask = tensor.tensor3('xp_mask', dtype='float32')
    # xp = xp.reshape([xp.shape[0], n_timesteps_xp,n_samples_xp*hist_len_xp])
    ...
    # word embedding for previous sentence
    emb_xp = tparams['Wemb_uhist'][xp.flatten()]
    emb_xp = emb_xp.reshape([n_timesteps_xp, n_samples_xp*hist_len_xp, options['dim_word']])
    ...
    if options['use_dropout']:
        ...
        rec_dropout_uttr_xp = theano.shared(numpy.array([retain_probability_hidden]*2, dtype='float32'))
        rec_dropout_hist_xp = theano.shared(numpy.array([retain_probability_hidden]*2, dtype='float32'))
        ...
        emb_dropout_uttr_xp = theano.shared(numpy.array([retain_probability_emb]*2, dtype='float32'))
        emb_dropout_hist_xp = theano.shared(numpy.array([retain_probability_emb]*2, dtype='float32'))
        ...
        source_dropout_xp = theano.shared(numpy.float32(retain_probability_source))
    ...
    # encoder='gru', gru_layer() init a set of parameters for foward rnn
    proj_uttr_xp = get_layer(options['encoder'])[1](tparams, emb_xp, options,
                                            prefix='encoder_uttr_xp', mask=xp_mask.reshape([n_timesteps_xp, n_samples_xp*hist_len_xp]),
                                            emb_dropout=emb_dropout_uttr_xp, rec_dropout=rec_dropout_uttr_xp)

    # proj_uttr_xp = proj_uttr_xp[0][-1]
    proj_uttr_xp = proj_uttr_xp[0]
    proj_uttr_xp = proj_uttr_xp.reshape([n_timesteps_xp, hist_len_xp, n_samples_xp, options['dim']])
    proj_hist_xp = proj_uttr_xp[-1]
    print 'proj_uttr_xp_dim:',proj_hist_xp.ndim
    ...
    proj_hist_xp = get_layer(options['encoder'])[1](tparams, proj_hist_xp, options,
                                            prefix='encoder_hist_xp', emb_dropout=emb_dropout_hist_xp, rec_dropout=rec_dropout_hist_xp)
    ...
    outs = [init_state, ctx, proj_hist_xp]
    f_init = theano.function([x, xp, xp_mask], outs, name='f_init', profile=profile)
    ...
    inps = [y, ctx, init_state, proj_hist_xp]
...
# gen_sample([f_init], [f_next], x[:, :, jj][:, :, None], trng=trng, k=1,maxlen=30, \
# stochastic=stochastic, argmax=False, suppress_unk=False)
def gen_sample(f_init,f_next,x,xp,xp_mask,
               trng=None, k=1, maxlen=30,stochastic=True,
               argmax=False, return_alignment=False, suppress_unk=False):
    ...
    #Longyue Add.
    h_ctx0 = [None]*num_models

    # get initial state of decoder rnn and encoder context
    for i in xrange(num_models):
        ret = f_init[i](x, xp,xp_mask)
        next_state[i] = ret[0]
        ctx0[i] = ret[1]
        h_ctx0[i] = ret[2]
    ...
    for ii in xrange(maxlen):
        for i in xrange(num_models):
            ...
            h_ctx = numpy.tile(h_ctx0[i], [live_k, 1])
            inps = [next_w, ctx, next_state[i], h_ctx]
            ret = f_next[i](*inps)
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=True, normalize=False, alignweights=False):
    ...
    for x, xp, y in iterator:
        ...
        x, x_mask, xp, xp_mask, hist_mask, y, y_mask = prepare_data(x, xp, y, options['hist_len'],
                                            n_words_src=options['n_words_src'],
                                            n_words=options['n_words'])

        ### in optional save weights mode.
        if alignweights:
            pprobs, attention = f_log_probs(x, x_mask, y, y_mask, xp, xp_mask, hist_mask)
            for jdata in get_alignments(attention, x_mask, y_mask):
                alignments_json.append(jdata)
        else:
            pprobs = f_log_probs(x, x_mask, y, y_mask, xp, xp_mask, hist_mask)
...
def train(dim_word=100,  # word vector dimensionality
    ...
    print '###### This is the Version (hnmt_src_static_gate_src_init_two)! ######'
    print use_dropout
    # Model options
    ...
    print [k for k,v in params.iteritems()]
    # reload parameters
    if reload_ and os.path.exists(saveto):
    ...
    trng, use_noise, \
        x, x_mask, y, y_mask, xp, xp_mask, hist_mask,\
        opt_ret, \
        cost,  hist_xp= \
        build_model(tparams, model_options)

    inps = [x, x_mask, y, y_mask, xp, xp_mask, hist_mask]
    f_print = theano.function(inps, [cost, hist_xp], name='f_print')
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, xp, y in train:
            ...
            x, x_mask, xp, xp_mask, hist_mask, y, y_mask = prepare_data(x, xp, y, hist_len, maxlen=maxlen,
            ...
            # cost, hist_xp = f_print(x, x_mask, y, y_mask, xp, xp_mask, hist_mask)
            # print hist_xp.shape

            cost = f_grad_shared(x, x_mask, y, y_mask, xp, xp_mask, hist_mask)
            ...
            if numpy.mod(uidx, sampleFreq) == 0:
                ...
                for jj in xrange(numpy.minimum(5, x.shape[2])):
                    ...
                    sample, score, sample_word_probs, alignment = gen_sample([f_init], [f_next],
                                               ...
                                               xp_mask[:, :, jj][:, :, None],
                                               trng=trng, k=1,
                                               maxlen=30, stochastic=stochastic,
                                               argmax=False,
                                               suppress_unk=False)
                    print 'Source ', jj, ': ',
            ...
            if numpy.mod(uidx, validFreq) == 0:
                ...
                if external_validation_script:
                    ...
                    print 'Saving  model...',
                    params = unzip(tparams)
                    saveto_new = saveto+'.'+str(uidx)
                    numpy.savez(saveto_new +'.dev', history_errs=history_errs, uidx=uidx, **params)
                    json.dump(model_options, open('%s.dev.npz.json' % saveto_new, 'wb'), indent=2)
                    print 'Done'
                    # p = Popen([external_validation_script,str(uidx)])
                    #with open("/home/vwang/Research/workspace/context-dialogue/script/context/tune.txt", "a") as myfile:
                    #    myfile.write(str(uidx)+"\n")
