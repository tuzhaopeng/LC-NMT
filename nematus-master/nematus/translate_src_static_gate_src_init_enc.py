...
def translate_model(models, options, k, normalize, verbose, nbest, return_alignment, suppress_unk, sent_list, xp_list, saveto,
                    word_idict_trg,print_word_probabilities,save_alignment,a_json):

    from hnmt_src_static_gate_src_init_enc import (build_sampler, gen_sample, load_params,
    ...
    def _translate(seq,seq_xp,xp_mask):
        ...
        sample, score, word_probs, alignment = gen_sample(fs_init, fs_next,
                                   ...
                                   numpy.array(seq_xp).T.reshape([1, len(seq_xp[0]), len(seq_xp), 1]),
                                   xp_mask.reshape([len(xp_mask[0]), len(xp_mask), 1]),
    ...
    for idx, (x, xp) in enumerate(zip(sent_list,xp_list)):
        print 'sent id:',idx
        # print xp
        lens = [len(i) for i in xp]
        max_len = max(lens)+1
        xp_mask = numpy.zeros((max_len, len(xp))).astype('float32')
        xp_new = []
        for id,i in enumerate(xp):
            xp_new.append(i+[[0]]*(max_len-len(i)))
            xp_mask[:len(i), id] = 1.
        xp = xp_new
        # print xp
        seq = _translate(x, xp, xp_mask)
        _calculate(idx, seq)
        # xp.append(result)
        # if len(xp)>3: xp = xp[-3:]
...
def process_file(f,chr_level,word_dicts,options):
    source_sentences,xp_sentences = [],[]
    for idx, line in enumerate(f):
        line_list = line.split('@@@@')

        if chr_level:
            words = list(line_list[1].decode('utf-8').strip())
            words_xp = list(line_list[0].decode('utf-8').strip())
        else:
            words = line_list[1].strip().split()
            words_xp = [i.strip().split() for i in line_list[0].split('####')]
        ...
        for w in words_xp:
            xp_tmp = []
            for ww in w:
                ww = [word_dicts[i][f] if f in word_dicts[i] else 1 for (i,f) in enumerate(ww.split('|'))]
                xp_tmp.append(ww)
                if len(ww) != options[0]['factors']:
                    sys.stderr.write('Error: expected {0} factors, but input word has {1}\n'.format(options[0]['factors'], len(w)))
                    # for midx in xrange(n_process):
                    #     processes[midx].terminate()
                    sys.exit(1)
        ...
        # xp += [[0]*options[0]['factors']]

        # queue.put((idx, x, xp))
        source_sentences.append(x)
        xp_sentences.append(xp)
    return idx+1, source_sentences, xp_sentences
...
def main(models, source_file, saveto, save_alignment, k=5,
    ...
    _, sent_list,xp_list = process_file(source_file,chr_level,word_dicts,options)
    # for idx,line in sent_list:
    translate_model(models, options, k, normalize, verbose, nbest, save_alignment is not None,
                    suppress_unk, sent_list, xp_list,saveto,word_idict_trg,
                    print_word_probabilities,save_alignment,a_json)
