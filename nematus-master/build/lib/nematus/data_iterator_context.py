    ...
    def next(self):
        ...
        try:
            ...
            while True:
                ...
                # print ss_string_list[0].strip(' ')
                sp = ss_string_list[0].split(' ')
                # print ' '.join(sp)
                for w in sp:
                    w = [self.source_dicts[i][f] if f in self.source_dicts[i] else 1 for (i,f) in enumerate(w.split('|'))]
                    tmp.append(w)
...
if __name__ == '__main__':
    DATA_DIR = "/home/vincent/Research/workspace/pycharm/NMT/nematus-master/wmt16-scripts-master/dialogue/data/"
    SRC = "zh"
    TGT = "en"
    VOCAB_SIZE = 976
    batch_size=10
